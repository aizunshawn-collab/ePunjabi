# Pure-Python port of IndicTransToolkit's IndicProcessor (upstream: processor.pyx).
# Ported instead of pip-installed because IndicTransToolkit ships a Cython extension
# that requires MSVC Build Tools (not available here) and is officially unsupported
# on Windows. The Cython version only used typed locals for speed - the actual logic
# is plain Python (regex, sacremoses, indic_nlp_library), so it behaves identically.
import regex as re
